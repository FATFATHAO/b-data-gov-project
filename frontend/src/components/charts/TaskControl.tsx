...
import { Card, Tabs, Input, Button, Form, message } from 'antd';
import { PlayCircleOutlined, VideoCameraAddOutlined } from '@ant-design/icons';
import { startLiveTask, startVideoTask } from '../../pages/api';
...
const TaskControl: React.FC<Props> = ({ enableVideo = true, platformName }) => {
  ...
  const handleLiveSubmit = async (values: { roomId: string }) => {
    setLoading(true);
    try {
      await startLiveTask(values.roomId, platformName.toLowerCase());
  ...
  const handleVideoSubmit = async (values: { bvId: string }) => {
    setLoading(true);
    try {
      await startVideoTask(values.bvId, platformName.toLowerCase());
